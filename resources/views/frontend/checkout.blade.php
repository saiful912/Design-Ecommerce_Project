            ...
            <div class="col-md-5 mt-5">
                <div class="card card-body mt-5">
                    ...
                    @foreach((new App\Models\Cart)->totalCarts() as $cart)
                        ...
                        <p>
                            <span class="badge badge-danger">{{$cart->product->title}}</span> -
                            <strong>{{number_format($cart->product->price,2)}} Taka</strong>
                            <span class="badge badge-info">{{$cart->product_quantity}} item</span>
                        </p>
                    ...
                    <h5 class="font-weight-bold text-secondary border-bottom">Total Price
                        : {{number_format($total_price,2)}} Tk</h5>
                    <h5 class="font-weight-bold text-secondary mt-2 border-bottom">Total price with shipping cost
                        : {{number_format($total_price + App\Models\Setting::first()->shipping_cost,2)}} Taka</h5>
                    <p>
                ...
                <div class="card card-body mt-5 mb-5">
                    ...
                    <form method="POST" action="{{ route('checkouts.store') }}">
                        ...
                        <div class="form-group row mt-2">
                            ...
                            <div class="col-md-6">
                                <input id="name" type="text"
                                       ...
                                       value="{{Auth::check() ? Auth::user()->name : ''}}" required autocomplete="name"
                                       autofocus>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <input id="phone_no" type="text"
                                       ...
                                       name="phone_no" value="{{Auth::check() ? Auth::user()->phone_no : ''}}"
                        ...
                        <textarea id="shipping_address"
                                  class="form-control @error('phone_no') is-invalid @enderror " rows="4"
                                    ...
                                    @else
                                        <div id="payment_{{$payment->short_name}}" class="hidden alert alert-success mt-2">
                                            ...
                                            <div class="alert alert-success">
                                                Please send the above money to this Number and write your transaction ID
                                            </div>
